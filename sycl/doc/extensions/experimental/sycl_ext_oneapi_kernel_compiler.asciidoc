...
* link:../proposed/sycl_ext_oneapi_syclbin.asciidoc[sycl_ext_oneapi_syclbin]
...
device images of state `bundle_state::object`, and a new kernel bundle is
created to contain these device images.
The new bundle represents all of the kernels in `sourceBundle` that are
compatible with at least one of the devices in `devs`.
Any remaining kernels (those that are not compatible with any of the devices in
...
class kernel_bundle {
  ...
  bool ext_oneapi_has_kernel(const std::string &name, const device &dev);
...
_Returns:_ The value `true` only if all the following hold:

 * The kernel bundle was created from a bundle of state
 `bundle_state::ext_oneapi_source` or from a
 link:../proposed/sycl_ext_oneapi_syclbin.asciidoc[SYCLBIN file];
 * and the kernel bundle defines a kernel whose name is `name`.
 ...
 * The kernel bundle was created from a bundle of state
 `bundle_state::ext_oneapi_source` or from a
 link:../proposed/sycl_ext_oneapi_syclbin.asciidoc[SYCLBIN file];
 * the kernel bundle defines a kernel whose name is `name`;
 * and the corresponding kernel is compatible with the device `dev`.
...
_Constraints:_ This function is only available when `State` is
`bundle_state::executable`.

_Returns:_ A pointer to the `device_global` whose name is `name` on device
`dev`. This pointer can be used in all interfaces that a USM pointer to device
memory can be used, excluding `sycl::free`. Dereferencing this pointer on host
...
_Constraints:_ This function is only available when `State` is
`bundle_state::ext_executable`.

_Returns:_ The size of the `device_global` whose name is `name` on device
`dev`.
