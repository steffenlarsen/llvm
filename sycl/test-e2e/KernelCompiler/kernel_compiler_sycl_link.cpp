//==- kernel_compiler_sycl_link.cpp --- kernel_compiler extension tests   --==//
//
...
extern "C" SYCL_EXTERNAL void TestFunc(int *Ptr, int Size);
...
int main() {
  ...
  if (!Q.get_device().ext_oneapi_can_compile(syclex::source_language::sycl)) {
    std::cout << "Device does not support SYCL source kernel bundle extension: "
  ...
  source_kb ImportSourceKB = syclex::create_kernel_bundle_from_source(
      Ctx, syclex::source_language::sycl, SYCLImportSource);
  source_kb ExportSourceKB = syclex::create_kernel_bundle_from_source(
      Ctx, syclex::source_language::sycl, SYCLExportSource);

  obj_kb ImportObjKB = syclex::compile(ImportSourceKB);
  obj_kb ExportObjKB = syclex::compile(
      ExportSourceKB,
      syclex::properties{syclex::build_options{
