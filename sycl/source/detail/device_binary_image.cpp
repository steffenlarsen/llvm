...
DynRTDeviceBinaryImage::DynRTDeviceBinaryImage(
  ...
  Data = std::unique_ptr<char[]>{new (std::align_val_t(
      PropertyAlignment)) char[PropertyCount * PaddedPropertyByteSize +
                               PropertyContentByteSize]};

  auto NextFreeProperty =
      reinterpret_cast<sycl_device_binary_property>(Data.get());
  char *NextFreeContent = Data.get() + (PropertyCount * PaddedPropertyByteSize);
