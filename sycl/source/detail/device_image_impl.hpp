...
public:
  ...
  device_image_impl(const RTDeviceBinaryImage *BinImage, context Context,
      ...
      : MBinImage(BinImage), MContext(std::move(Context)),
        ...
        MBinaryStructStorage(std::move(BinaryStructStorage)) {}
...
private:
  ...
  // Used to store the data detailing intermediately created binaries, such as
  // the information about linked binaries.
  std::vector<unsigned char> MBinaryStructStorage;
};
